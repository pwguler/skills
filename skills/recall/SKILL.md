...
- Never take a pick without the reason: ask for it with every probe, one line. The pick is cheap and can be a guess; the reason is what gets scored against the code, and a hollow one shows the gap the pick hid.
...
- When the questions run out, report the gaps found, in a few lines, and stop. Record nothing. A generated document substitutes the appearance of understanding for the real thing. If something durable emerges, it is a `CONTEXT.md` term per [TERMS.md](../drill/TERMS.md) or an ADR per [DECISION-RECORD.md](../drill/DECISION-RECORD.md), and the user decides.
