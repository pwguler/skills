...
- **No shared mutable state across an async seam.** What passes between tasks, threads, workers, or requests is a message or an immutable value, not a reference both sides mutate. Ownership moves; it is not shared.
- **Name every async seam.** Where work becomes concurrent (a queue, a goroutine, a promise fan-out, a background job) is a seam in LANGUAGE.md's sense: it is documented, and what crosses it is typed.
