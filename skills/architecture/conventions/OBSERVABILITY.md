...
- **No secrets, no PII in logs.** Redact at the emitting seam, not downstream; the log pipeline is not trusted.
...
- **Errors are observable where they are handled.** An expected failure is logged once, at the seam that decides what to do about it, with the correlation id; never logged at every layer it passes through.
