...
- **The seam is a port the domain owns.** The domain declares what it needs (an interface in its own vocabulary); the edge supplies an adapter. DEPENDENCIES.md's dependency categories decide whether that port is internal or at the module's external interface.
