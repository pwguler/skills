...
- **Validate at the edge.** Parse and validate every inbound payload there; inside the service, types are already trusted. Never trust a client value.
...
- **Logs and secrets.** Structured logs at the edge, no secrets in them. Configuration comes from the environment, never the codebase.
