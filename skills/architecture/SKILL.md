...
The conventions this skill owns, loaded when the work touches them. Universal, for any code: [TYPES.md](conventions/TYPES.md) (illegal states unrepresentable), [FAILURE.md](conventions/FAILURE.md) (failure as part of the interface), [DIRECTION.md](conventions/DIRECTION.md) (dependencies point inward), [PURITY.md](conventions/PURITY.md) (immutable data, pure core, effects at the edge), [CONCURRENCY.md](conventions/CONCURRENCY.md) (one owner per state, named async seams), [SECURITY.md](conventions/SECURITY.md) (trust nothing that crossed a seam), [OBSERVABILITY.md](conventions/OBSERVABILITY.md) (diagnosable by design), [COHESION.md](conventions/COHESION.md) (one reason to change, named from CONTEXT.md). By surface: [FRONTEND.md](conventions/FRONTEND.md) for UI built from a settled design, [BACKEND.md](conventions/BACKEND.md) for APIs, services, and jobs. Per toolchain and stack: [tooling/](tooling/README.md). The look itself lives in the project's `DESIGN.md`, which `prototype` grounds in and FRONTEND.md reads. `implement` follows all of them while building.
