...
1. Run the `verify` skill. When a spec exists at `docs/specs/<slug>.md`, the final gate is a fresh judge that reads only the spec and the diff and reports criterion by criterion, checking the diff against the repo's agent conventions (CLAUDE.md or AGENTS.md, whatever the harness names one) and against `ARCHITECTURE.md` and `CONTEXT.md` when they exist; the maker does not certify its own work. A subagent is the judge when the harness dispatches one; without dispatch, hand the spec and the diff to the user as the gate and say so. Failing work does not reach the options menu.
