...
Integrate when they return: read each summary, check the diffs for conflicts, then run the `verify` skill, whose full gate covers the suite, before claiming the batch done. A batch is the biggest slice you did not write; `/recall` after integration pays the understanding back.
