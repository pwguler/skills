...
3. Run the commands that would prove it false: the tests, the build, the linter, and the actual behavior that changed. The full gate (every test, the build, the linter) runs once per commit, on the tree about to be committed; inside a slice, the red-green loop runs only the tests that exercise it.
